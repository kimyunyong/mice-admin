	...
	<section class="sect_nonetit">
		...
		<table class="adm_tbl2 mobile_none" summary="권한관리에  관한 테이블입니다.권한ID,권한 명,설명, 등록일자, 롤 정보의 내용을 담고 있습니다.">
			...
			<c:forEach var="result" items="${resultList}" varStatus="status">
				<tr>
					<td>${result.idx}</td>
					<td><a href="/cvb/form/event_info.do?idx=${result.idx}">${result.event_name_ko}</a></td>
...
<!-- 모바일에서만 보이는 리스트 -->
			<c:forEach var="result" items="${resultList}" varStatus="status">
					<div class="mobile_list">
						...
						<dl>
							<dt>행사명</dt>
							<dd><a href="/cvb/form/event_info.do?idx=${result.idx}">${result.event_name_ko}</dd>
						</dl>
						<dl>
							<dt>연락처</dt>
