...
//		return "egovframework/com/cmm/EgovUnitMain";
	...
	@ResponseBody
	public String index3(ModelMap model) throws Exception {
		
		String encrypt = com.mice.common.encrypt.AESGenerator.encrypt("aaa");
		String decrypt = com.mice.common.encrypt.AESGenerator.decrypt("테스트1");
		return "<html><body><div>encrypt:" +encrypt + "</div><div>" + decrypt + "</div>";
	}
	@RequestMapping("/EgovTop__.do")
	public String top() {
	...
	public String setContent(ModelMap model,HttpSession session) {
		...
		UserManageVO vo = service.teamPositionById(loginVO.getId());
		
		session.setAttribute("team_idx", vo.getTeam_idx());
		session.setAttribute("position", vo.getPosition());
		
		java.util.List<String> lst = EgovUserDetailsHelper.getAuthorities();
//		return "egovframework/com/cmm/EgovUnitContent";
		return "mice/main/main";
	}
