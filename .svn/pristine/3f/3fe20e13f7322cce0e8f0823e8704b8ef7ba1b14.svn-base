...
function chkExcel(frm, val, flag){
	...
	if(flag == 0) frm = document.frm_top_search;
	if(flag == 1){
		if(val == "select"){
	...
	alert(flag +"____"+val+"____"+$(frm).find("input[name=searchNum]").is(":checked"));
	...
	//location.href="/crm/search/insert_action.do?"+params;
	//alert(params);
...
function chkSMS(frm, val, flag){
	...
	var params = $(frm).serialize(); 
	window.open("/crm/search/sms_msg.do?"+params, "sms", "width=800px, height=600px, resizable=yes, scrollbars=yes");

	//window.open("", "SendSms", "width=1200px, height=800px, resizable=yes, scrollbars=yes");
	//$("form[name=form]").target = "SendSms";
	//$("form[name=form]").action = "/crm/search/sms_msg.do?"+params;
	//$("form[name=form]").submit();	
...
function chkReset(frm){
		...
		$(frm).find("input[name=searchMngName]").val("");
...
$(document).ready(function(){
	...
	var str1 = "form input[name=searchUserName], form input[name=searchSosoc], form input[name=searchEvtName], "
	str1 = str1 + "form input[name=searchEvtName],form input[name=searchArea] ,form input[name=searchRegName],";
	str1 = str1 + "form input[name=searchUserAddr], form input[name=searchUserPosition], form input[name=searchUserEtc]";
	...
	var str2 = "form_mobile input[name=searchUserName], form_mobile input[name=searchSosoc], form_mobile input[name=searchEvtName], "
	str2 = str2 + "form_mobile input[name=searchEvtName],form_mobile input[name=searchArea] ,form_mobile input[name=searchRegName],";
	str2 = str2 + "form_mobile input[name=searchUserAddr], form_mobile input[name=searchUserPosition], form_mobile input[name=searchUserEtc]";
...
<div class="mobile_none">	
	<form name="form"  action="/crm/search/list.do" method="get" enctype="multipart/form-data" onsubmit="return true;">
		<input type="hidden" name="flag" value="${flag}" />
...
<!-- 180111 -->
		...
		<div class="btn_right_area">
			...
			<c:if test="${flag eq '1'}"><a href="javascript:chkExcel(document.form,'select','${flag}');" class="sbtn01"  id="btnExcelSelect">선택항목 엑셀다운로드</a></c:if>
			<c:if test="${flag eq '1'}"><a href="javascript:chkSMS(document.form,'select','${flag}');" class="sbtn01"  id="btnExcelSelect">선택항목 문자보내기</a></c:if>
	...
	<!-- //180111 -->	
			...
			<thead>
			  <tr>
				...
				<th>
					...
					<ul>
						<li><a href="javascsript:chkOrder(document.form,'name_asc');">▲</a></li>
		...
		<c:if test="${top_cnt==0 || top_cnt ==1 }">
			  <tr>
				...
				<td><c:out value="${(vo.pageIndex-1) * vo.recordCountPerPage + status.count}"/>
				</td>
				<td>
		...
		<c:if test="${top_cnt>1 }">
			  <tr>
				...
				<td rowspan="${top_cnt}">
					...
					<strong class="name"><a href="/crm/customer/info.do?idx=${result.cus_idx}" target="_blank">${result.name_ko}</a>
					<c:if test="${result.position != ''}">(${result.position})</c:if>
					</strong>
					<p class="detail_i_st01"><i class="fa fa-phone iconin01" aria-hidden="true" ></i>${result.tel_no}</p>
					<p class="detail_i_st01"><i class="fa fa-mobile iconin02" aria-hidden="true"></i>${d:decrypt(result.phone_no)}</p>
					<p class="detail_i_st01"><i class="fa fa-envelope-o iconin03" aria-hidden="true"></i>${d:decrypt(result.email)}</p>
...
<div class="mobile_show">
	<form name="form_mobile"  action="/crm/search/list.do" method="get" enctype="multipart/form-data" onsubmit="return true;">
		...
		<div class="search_new_btn ">
			...
			<c:if test="${flag eq '1'}"><a href="javascript:chkExcel(document.form_mobile,'select','${flag}');" class="sbtn01 mobilebtn02"  id="btnExcelSelect">선택 <i class="fa fa-file-excel-o" aria-hidden="true"></i></a></c:if>
			<c:if test="${flag eq '1'}"><a href="javascript:chkSMS(document.form_mobile,'select','${flag}');" class="sbtn01 mobilebtn02"  id="btnExcelSelect">선택<i class="fa fa-envelope-o" aria-hidden="true"></i></a></c:if>

			<a href="javascript:chkExcel(document.form_mobile,'search','${flag}');" class="sbtn01 mobilebtn02"  id="btnExcelSearch">검색 <i class="fa fa-file-excel-o" aria-hidden="true"></i></a>
			<a href="javascript:chkSMS(document.form_mobile,'search','${flag}');" class="sbtn01 mobilebtn02"  id="btnExcelSearch">검색 <i class="fa fa-envelope-o" aria-hidden="true"></i></a>

			<a href="javascript:chkExcel(document.form_mobile,'all','${flag}');" class="sbtn01 mobilebtn02"  id="btnExcelAll">전체 <i class="fa fa-file-excel-o" aria-hidden="true"></i></a>
			<a href="javascript:chkSMS(document.form_mobile,'all','${flag}');" class="sbtn01 mobilebtn02"  id="btnExcelAll">전체 <i class="fa fa-envelope-o" aria-hidden="true"></i></a>
		...
		<c:set value = "${fn:length(result.eventSubContList)}" var="top_cnt" />	
			...
			<div class="list_wrapper">
				<dl class="cin01">
					<dt></dt>
					<dd><input type="checkbox" name="searchNum" value="${result.cus_idx}"  checked /></dd>
				</dl>
				<dl class="cin01">
					<dt>성명</dt>
					<dd>${result.name_ko}<c:if test="${result.position != ''}">(${result.position})</c:if>
						<p class="detail_i_st01"><i class="fa fa-phone iconin01" aria-hidden="true" ></i>${result.tel_no}</p>
						<p class="detail_i_st01"><i class="fa fa-mobile iconin02" aria-hidden="true"></i>${d:decrypt(result.phone_no)}</p>
						<p class="detail_i_st01"><i class="fa fa-envelope-o iconin03" aria-hidden="true"></i>${d:decrypt(result.email) }</p>
					</dd>
		...
		<div id="pagination_info_mobile" align="center">
		    <div>
		        <ui:pagination paginationInfo = "${paginationInfo}"
		            type="text"
