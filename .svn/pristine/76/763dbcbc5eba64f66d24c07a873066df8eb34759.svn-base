...
import com.mice.crm.service.impl.KMSDAO;
...
public class EgovUserManageServiceImpl extends EgovAbstractServiceImpl implements EgovUserManageService {
	...
	public String insertUser(UserManageVO userManageVO) throws Exception {
		...
		// 팀을 선택하지 않을 경우
		if(userManageVO.getTeam_idx().equals(null) || userManageVO.getTeam_idx().equals("")){
			userManageVO.setTeam_idx("-1");
		}
		...
		userManageVO.setReg_id(user.getId());
		
		kmsDao.insertDirEmp(userManageVO);
	...
	@Override
	   public void updateUser(UserManageVO userManageVO) throws Exception {
	      if(userManageVO.getTeam_idx().equals("")){
	         userManageVO.setTeam_idx("-1");
	      }
	      ...
	      UserManageVO getUser = kmsDao.selectUser(userManageVO.getEmplyrId());
	      	      
	      userManageDAO.updateUser(userManageVO);
	      
	      // ----------------------------------------------------------------------------
	      
	      if(!getUser.getTeam_idx().equals(userManageVO.getTeam_idx())){
	         kmsDao.updateAllDirUseFalse(userManageVO);   // 사용자의 모든 디렉토리를 use_tf = 0한다.
	         kmsDao.deleteFileById(userManageVO.getEmplyrId());
	         kmsDao.insertDirEmp(userManageVO);   // 없으면 directory에 바뀐 팀으로 직원을 생성한다.
	      }   
	      
//	      int count = kmsDao.getDirCountByTeamAndEmp(userManageVO);   // directory에 팀에 해당하는 직원이 있는지 확인한다.
	//
//	      if(count > 0){
//	         kmsDao.updateDirUseTrue(userManageVO);   // 있으면 directory에 기존 팀을 use_tf = 1 한다.
//	         kmsDao.updateDirEmpName(userManageVO);
//	      }else{
//	         
